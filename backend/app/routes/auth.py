...
def login():
    ...
    response = jsonify(
        msg="You have successfully logged in", access_token=access_token
    )
...
def forgot_password():
    ...
    reset_url = url_for(
        'main.auth_bp.reset_password', token=reset_token, _external=True
    )
...
def reset_password(token):
    ...
    tenantsCollection.update_one(
        {
            "contact_details.email": email}, {"$set": {

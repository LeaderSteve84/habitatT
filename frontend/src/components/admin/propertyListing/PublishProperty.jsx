...
export default function PublishProperty() {
  ...
  const submitForm = async (e) => {
    ...
    try {
      // route: advert-properties
      const response = await axios.post('/api/properties', JSON.stringify(formData), {
        headers: {
      ...
      console.log("Advertised Property added successfully:", response.data);
    } catch (err) {
      console.error("Error adding advert property:", err);
    }
  };
  ...
  return (
    <div className="flex justify-center items-center h-screen bg-gray-100">
      <form className="bg-white p-8 rounded-lg shadow-md w-96" onSubmit={submitForm}>
        <h2 className="text-2xl font-bold mb-6 text-center">Publish Property</h2>
        <div className="mb-4">
